...
public class AudioManager : SingletonBaseDontDestroyOnLoad<AudioManager>
{
    ...
    [SerializeField] private AudioSource eletricSound;
    private void Start()
    {
        if (!GameCache.GC.soundOff) bgMusic.Play();
    }
    public void SetEletricSound(AudioClip sound)
    {
        eletricSound.clip = sound;
    }
    public void PlayEletricSound(bool play)
    {
        if (play) eletricSound.Play();
        else eletricSound.Stop();
    }
