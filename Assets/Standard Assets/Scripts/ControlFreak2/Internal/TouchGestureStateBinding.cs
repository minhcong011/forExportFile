// dnSpy decompiler from Assembly-CSharp-firstpass.dll class: ControlFreak2.Internal.TouchGestureStateBinding
using System;
using UnityEngine;
...
namespace ControlFreak2.Internal
{
	...
	public class TouchGestureStateBinding : InputBindingBase
	{
			...
			if (orientedState.PressedRaw())
			{
				...
				if (orientedState.PressedLong())
				{
					...
					if (screenState.Swiped())
					{
						...
						this.longPressSwipeHorzAxisBinding.SyncFloat(constrainedDeltaVec.x, InputRig.InputSource.TouchDelta, rig);
						this.longPressSwipeVertAxisBinding.SyncFloat(constrainedDeltaVec.y, InputRig.InputSource.TouchDelta, rig);
					}
				...
				else if (orientedState.PressedNormal())
				{
					...
					if (screenState.Swiped())
					{
						...
						this.normalPressSwipeHorzAxisBinding.SyncFloat(constrainedDeltaVec.x, InputRig.InputSource.TouchDelta, rig);
						this.normalPressSwipeVertAxisBinding.SyncFloat(constrainedDeltaVec.y, InputRig.InputSource.TouchDelta, rig);
					}
					if (swipeJoyState != null)
					{
		...
		{
			return this.enabled && (this.rawPressBinding.IsBoundToAxis(axisName, rig) || this.normalPressBinding.IsBoundToAxis(axisName, rig) || this.longPressBinding.IsBoundToAxis(axisName, rig) || this.tapBinding.IsBoundToAxis(axisName, rig) || this.doubleTapBinding.IsBoundToAxis(axisName, rig) || this.longTapBinding.IsBoundToAxis(axisName, rig) || this.normalPressSwipeDirBinding.IsBoundToAxis(axisName, rig) || this.longPressSwipeDirBinding.IsBoundToAxis(axisName, rig) || this.normalPressSwipeHorzAxisBinding.IsBoundToAxis(axisName, rig) || this.normalPressSwipeVertAxisBinding.IsBoundToAxis(axisName, rig) || this.longPressSwipeHorzAxisBinding.IsBoundToAxis(axisName, rig) || this.longPressSwipeVertAxisBinding.IsBoundToAxis(axisName, rig) || this.normalPressScrollHorzBinding.IsBoundToAxis(axisName, rig) || this.normalPressScrollVertBinding.IsBoundToAxis(axisName, rig) || this.longPressScrollHorzBinding.IsBoundToAxis(axisName, rig) || this.longPressScrollVertBinding.IsBoundToAxis(axisName, rig) || this.rawPressMousePosBinding.IsBoundToAxis(axisName, rig) || this.normalPressMousePosBinding.IsBoundToAxis(axisName, rig) || this.longPressMousePosBinding.IsBoundToAxis(axisName, rig) || this.tapMousePosBinding.IsBoundToAxis(axisName, rig) || this.doubleTapMousePosBinding.IsBoundToAxis(axisName, rig) || this.longTapMousePosBinding.IsBoundToAxis(axisName, rig) || this.normalPressSwipeMousePosBinding.IsBoundToAxis(axisName, rig) || this.longPressSwipeMousePosBinding.IsBoundToAxis(axisName, rig) || this.longPressSwipeJoyBinding.IsBoundToAxis(axisName, rig) || this.normalPressSwipeJoyBinding.IsBoundToAxis(axisName, rig) || this.rawPressEmuTouchBinding.IsBoundToAxis(axisName, rig) || this.normalPressEmuTouchBinding.IsBoundToAxis(axisName, rig) || this.longPressEmuTouchBinding.IsBoundToAxis(axisName, rig));
		}

		protected override bool OnIsBoundToKey(KeyCode keyCode, InputRig rig)
		{
			return this.enabled && (this.rawPressBinding.IsBoundToKey(keyCode, rig) || this.normalPressBinding.IsBoundToKey(keyCode, rig) || this.longPressBinding.IsBoundToKey(keyCode, rig) || this.tapBinding.IsBoundToKey(keyCode, rig) || this.doubleTapBinding.IsBoundToKey(keyCode, rig) || this.longTapBinding.IsBoundToKey(keyCode, rig) || this.normalPressSwipeDirBinding.IsBoundToKey(keyCode, rig) || this.longPressSwipeDirBinding.IsBoundToKey(keyCode, rig) || this.normalPressSwipeHorzAxisBinding.IsBoundToKey(keyCode, rig) || this.normalPressSwipeVertAxisBinding.IsBoundToKey(keyCode, rig) || this.longPressSwipeHorzAxisBinding.IsBoundToKey(keyCode, rig) || this.longPressSwipeVertAxisBinding.IsBoundToKey(keyCode, rig) || this.longPressSwipeJoyBinding.IsBoundToKey(keyCode, rig) || this.normalPressSwipeJoyBinding.IsBoundToKey(keyCode, rig) || this.normalPressScrollHorzBinding.IsBoundToKey(keyCode, rig) || this.normalPressScrollVertBinding.IsBoundToKey(keyCode, rig) || this.longPressScrollHorzBinding.IsBoundToKey(keyCode, rig) || this.longPressScrollVertBinding.IsBoundToKey(keyCode, rig));
		}

		protected override bool OnIsEmulatingMousePosition()
		{
			return this.rawPressMousePosBinding.IsEmulatingMousePosition() || this.normalPressMousePosBinding.IsEmulatingMousePosition() || this.longPressMousePosBinding.IsEmulatingMousePosition() || this.tapMousePosBinding.IsEmulatingMousePosition() || this.doubleTapMousePosBinding.IsEmulatingMousePosition() || this.longTapMousePosBinding.IsEmulatingMousePosition() || this.normalPressSwipeMousePosBinding.IsEmulatingMousePosition() || this.longPressSwipeMousePosBinding.IsEmulatingMousePosition() || this.longPressSwipeJoyBinding.IsEmulatingMousePosition() || this.normalPressSwipeJoyBinding.IsEmulatingMousePosition() || this.rawPressEmuTouchBinding.IsEmulatingMousePosition() || this.normalPressEmuTouchBinding.IsEmulatingMousePosition() || this.longPressEmuTouchBinding.IsEmulatingMousePosition();
		}

		protected override bool OnIsEmulatingTouches()
		{
			return this.rawPressMousePosBinding.IsEmulatingTouches() || this.normalPressMousePosBinding.IsEmulatingTouches() || this.longPressMousePosBinding.IsEmulatingTouches() || this.tapMousePosBinding.IsEmulatingTouches() || this.doubleTapMousePosBinding.IsEmulatingTouches() || this.longTapMousePosBinding.IsEmulatingTouches() || this.normalPressSwipeMousePosBinding.IsEmulatingTouches() || this.longPressSwipeMousePosBinding.IsEmulatingTouches() || this.longPressSwipeJoyBinding.IsEmulatingTouches() || this.normalPressSwipeJoyBinding.IsEmulatingTouches() || this.rawPressEmuTouchBinding.IsEmulatingTouches() || this.normalPressEmuTouchBinding.IsEmulatingTouches() || this.longPressEmuTouchBinding.IsEmulatingTouches();
		}
