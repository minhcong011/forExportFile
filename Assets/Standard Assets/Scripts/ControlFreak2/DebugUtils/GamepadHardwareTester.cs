// dnSpy decompiler from Assembly-CSharp-firstpass.dll class: ControlFreak2.DebugUtils.GamepadHardwareTester
using System;
using UnityEngine;
...
namespace ControlFreak2.DebugUtils
{
	...
	public class GamepadHardwareTester : MonoBehaviour
	{
		...
		{
			return UnityEngine.Input.GetKey(KeyCode.Joystick1Button0 + key);
		}
		...
		private void OnGUI()
		{
			...
			for (int i = 0; i < 4; i++)
			{
				...
				else
				{
					for (int j = 0; j < 10; j++)
					{
						float axisRaw = UnityEngine.Input.GetAxisRaw(GamepadHardwareTester.GetJoyAxisName(i, j));
					...
					for (int k = 0; k < 20; k++)
					{
						if (UnityEngine.Input.GetKey(GamepadHardwareTester.GetJoyKeyCode(i, k)))
						{
							string text2 = text;
				...
				float num = (float)((Screen.width - 20) / 4);
				GUI.Box(new Rect(10f + (float)i * num + 5f, 10f, num - 5f, (float)(Screen.height - 130)), text);
			}
			...
			GUI.Box(new Rect(10f, (float)(Screen.height - 100), (float)(Screen.width - 20), 50f), text3);
			if (GUI.Button(new Rect(10f, (float)(Screen.height - 40), 100f, 30f), "Reset Axes"))
			{
				Input.ResetInputAxes();
			}
			if (GUI.Button(new Rect(115f, (float)(Screen.height - 40), 100f, 30f), "Exit"))
			{
				Application.Quit();
			}
			GUI.Box(new Rect(220f, (float)(Screen.height - 40), (float)(Screen.width - 230), 30f), "Unity Ver [" + Application.unityVersion + "]");
		}
