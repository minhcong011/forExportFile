// dnSpy decompiler from Assembly-UnityScript-firstpass.dll class: ThirdPersonController
using System;
using UnityEngine;
...
public class ThirdPersonController : MonoBehaviour
{
	...
	public virtual void Awake()
	{
		...
		if (!this._animation)
		{
			UnityEngine.Debug.Log("The character you would like to control doesn't have animations. Moving her might look weird.");
		}
		if (!this.idleAnimation)
		{
			this._animation = null;
			UnityEngine.Debug.Log("No idle animation found. Turning off animations.");
		}
		if (!this.walkAnimation)
		{
			this._animation = null;
			UnityEngine.Debug.Log("No walk animation found. Turning off animations.");
		}
		if (!this.runAnimation)
		{
			this._animation = null;
			UnityEngine.Debug.Log("No run animation found. Turning off animations.");
		}
		if (!this.jumpPoseAnimation && this.canJump)
		{
			this._animation = null;
			UnityEngine.Debug.Log("No jump animation found and the character has canJump enabled. Turning off animations.");
		}
	}
	...
	{
		return Mathf.Abs(UnityEngine.Input.GetAxisRaw("Vertical")) + Mathf.Abs(UnityEngine.Input.GetAxisRaw("Horizontal")) > 0.5f;
	}
