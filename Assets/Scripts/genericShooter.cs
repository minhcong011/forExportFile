// dnSpy decompiler from Assembly-CSharp.dll class: genericShooter
using System;
using System.Collections;
using ControlFreak2;
using UnityEngine;

public class genericShooter : FPSCommonWeaponSpecs
{
	private void Awake()
	{
	...
	private void Update()
	{
		...
		if (this.currentammo == 0 || this.currentammo <= 0)
		{
			if (this.ammo <= 0)
			{
				...
				if ((CF2Input.GetButton("Fire1") || (double)CF2Input.GetAxis("Fire1") > 0.1 || this.weaponfirer.rayCastFirerAutoFireAim) && !this.myAudioSource.isPlaying && !this.myAudioSource.isPlaying)
				{
