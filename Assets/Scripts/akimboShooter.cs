// dnSpy decompiler from Assembly-CSharp.dll class: akimboShooter
using System;
using System.Collections;
using ControlFreak2;
using UnityEngine;

public class akimboShooter : FPSCommonWeaponSpecs
{
	private void Awake()
	{
	...
	private void Update()
	{
		...
		if (this.retract)
		{
			this.canfire = false;
			this.canaim = false;
			base.transform.localPosition = Vector3.MoveTowards(base.transform.localPosition, this.retractPos, num);
			this.weaponnextfield = this.weaponnormalFOV;
			this.nextField = this.normalFOV;
		}
		else if (this.playercontrol.running && !this.retract)
		{
			this.canfire = false;
	...
	private void onstart()
	{
		...
		this.fireAudioSource1.Stop();
		this.fireAudioSource2.Stop();
	...
	private void fire()
	{
		if (!this.myanimation.isPlaying)
		{
			if (!this.fireleft)
			{
				...
				base.StartCoroutine(this.flashthemuzzle1());
				this.weaponfirer.fire();
				this.fireAudioSource1.clip = this.fireSound;
				this.fireAudioSource1.pitch = 0.9f + 0.1f * UnityEngine.Random.value;
				this.fireAudioSource1.Play();
				this.myanimation[this.fireAnim1.name].speed = this.fireAnimSpeed;
				this.myanimation.Play(this.fireAnim1.name);
				this.currentammo--;
				base.StartCoroutine(this.ejectshell1(this.shellejectdelay));
				this.fireleft = true;
			}
			else
			{
				float num2 = UnityEngine.Random.Range(-0.05f, -0.01f);
				base.transform.localPosition = new Vector3(base.transform.localPosition.x, base.transform.localPosition.y, base.transform.localPosition.z + num2);
				base.StartCoroutine(this.flashthemuzzle2());
				this.weaponfirer.fire();
				this.fireAudioSource2.clip = this.fireSound;
				this.fireAudioSource2.pitch = 0.9f + 0.1f * UnityEngine.Random.value;
				this.fireAudioSource2.Play();
				this.myanimation[this.fireAnim2.name].speed = this.fireAnimSpeed;
				this.myanimation.Play(this.fireAnim2.name);
				this.currentammo--;
				base.StartCoroutine(this.ejectshell2(this.shellejectdelay));
				this.fireleft = false;
			}
	...
	private IEnumerator ejectshell1(float waitTime)
	{
		yield return new WaitForSeconds(waitTime);
		GameObject shellInstance = UnityEngine.Object.Instantiate<GameObject>(this.shell, this.shellPos1.transform.position, this.shellPos1.transform.rotation);
	...
	private IEnumerator ejectshell2(float waitTime)
	{
		yield return new WaitForSeconds(waitTime);
		GameObject shellInstance = UnityEngine.Object.Instantiate<GameObject>(this.shell, this.shellPos2.transform.position, this.shellPos2.transform.rotation);
		shellInstance.GetComponent<Rigidbody>().AddRelativeForce(30f, 80f, 0f);
		shellInstance.GetComponent<Rigidbody>().AddRelativeTorque(500f, 20f, 800f);
		yield break;
	}
	...
	private IEnumerator flashthemuzzle1()
	{
		ParticleSystem smoke = this.muzzlesmoke1.GetComponent<ParticleSystem>();
		smoke.Emit(1);
		this.muzzle1.transform.localEulerAngles = new Vector3(0f, 0f, UnityEngine.Random.Range(0f, 360f));
		this.muzzle1.gameObject.SetActive(true);
		yield return new WaitForSeconds(0.05f);
		this.muzzle1.gameObject.SetActive(false);
		yield break;
	}

	private IEnumerator flashthemuzzle2()
	{
		ParticleSystem smoke2 = this.muzzlesmoke2.GetComponent<ParticleSystem>();
		smoke2.Emit(1);
		this.muzzle2.transform.localEulerAngles = new Vector3(0f, 0f, UnityEngine.Random.Range(0f, 360f));
		this.muzzle2.gameObject.SetActive(true);
		yield return new WaitForSeconds(0.05f);
		this.muzzle2.gameObject.SetActive(false);
		yield break;
	}
