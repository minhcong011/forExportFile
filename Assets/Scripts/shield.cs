// dnSpy decompiler from Assembly-CSharp.dll class: shield
using System;
...
public class shield : MonoBehaviour
{
	...
	private void Start()
	{
		...
		this.lastguarding = this.isguarding;
	...
	private void Update()
	{
		...
		else
		{
			...
			if ((CF2Input.GetButton("Aim") || (double)CF2Input.GetAxis("Aim") > 0.1) && this.canaim && !this.playercontrol.running)
			{
		...
		if (this.isguarding != this.lastguarding)
		{
			base.StartCoroutine(this.doguard());
		}
	...
	private IEnumerator doguard()
	{
		if (this.isguarding)
		{
			if (!this.isreloading)
			{
				this.myanimation.Play(this.guardAnim.name);
				yield return new WaitForSeconds(base.GetComponent<Animation>()[this.guardAnim.name].length);
			}
		}
		else if (!this.isreloading)
		{
			this.myanimation.Play(this.unguardAnim.name);
			yield return new WaitForSeconds(base.GetComponent<Animation>()[this.unguardAnim.name].length);
		}
		yield break;
	}
	...
	public AnimationClip guardAnim;

	public AnimationClip unguardAnim;
