// dnSpy decompiler from Assembly-CSharp.dll class: UIDrawCall
using System;
	...
	public void UpdateGeometry()
	{
		...
		if (size > 0 && size == this.uvs.size && size == this.cols.size && size % 4 == 0)
		{
			...
			else
			{
				...
				UnityEngine.Debug.LogError("Too many vertices on one panel: " + this.verts.size);
			}
			if (this.mRenderer == null)
			{
			...
			while (parentPanel != null)
			{
				if (parentPanel.hasClipping)
				{
					...
					if (parentPanel != this.panel)
					{
						...
						{
							UnityEngine.Debug.LogWarning("Panel can only be clipped properly if X and Y rotation is left at 0", this.panel);
						}
						angle = vector2.z;
					}
