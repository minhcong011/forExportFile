// dnSpy decompiler from Assembly-CSharp.dll class: Thumper
using System;
using System.Collections;
using ControlFreak2;
using UnityEngine;

public class Thumper : FPSCommonWeaponSpecs
{
	private void Awake()
	{
	...
	private void reload()
	{
		if (!this.myanimation.isPlaying && this.canreload && !this.isreloading)
		{
			if (this.currentammo >= 1)
			{
				...
				base.StartCoroutine(this.setreload2(this.myanimation[this.reload2Anim.name].length));
	...
	private IEnumerator setreload(float waitTime)
	{
		...
		this.currentammo += Mathf.Clamp(this.clipSize / 2, 0, this.ammo);
		this.ammo--;
	...
	private IEnumerator setreload2(float waitTime)
	{
		this.playercontrol.canclimb = false;
		this.inventory.canswitch = false;
		this.canaim = false;
	...
	private IEnumerator flashthemuzzle()
	{
		if (this.currentammo == 1)
		{
			ParticleSystem smoke = this.muzzlesmokeright.GetComponent<ParticleSystem>();
			smoke.Emit(1);
			this.muzzleRight.transform.localEulerAngles = new Vector3(0f, 0f, UnityEngine.Random.Range(0f, 360f));
			this.muzzleRight.gameObject.SetActive(true);
			yield return new WaitForSeconds(0.05f);
			this.muzzleRight.gameObject.SetActive(false);
		}
		else
		{
			ParticleSystem smoke2 = this.muzzlesmokeleft.GetComponent<ParticleSystem>();
			smoke2.Emit(1);
			this.muzzleLeft.transform.localEulerAngles = new Vector3(0f, 0f, UnityEngine.Random.Range(0f, 360f));
			this.muzzleLeft.gameObject.SetActive(true);
			yield return new WaitForSeconds(0.05f);
			this.muzzleLeft.gameObject.SetActive(false);
		}
		yield break;
	}
