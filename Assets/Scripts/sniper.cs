// dnSpy decompiler from Assembly-CSharp.dll class: sniper
using System;
using System.Collections;
using ControlFreak2;
using UnityEngine;

public class sniper : FPSCommonWeaponSpecs
{
	private void Awake()
	{
	...
	private IEnumerator setfire()
	{
		if (this.currentammo > 1)
		{
			...
			float randomZ = UnityEngine.Random.Range(-0.05f, -0.01f);
			base.transform.localPosition = new Vector3(base.transform.localPosition.x, base.transform.localPosition.y, base.transform.localPosition.z + randomZ);
			this.myanimation.Play(this.fireAnim.name);
			this.currentammo--;
			yield return new WaitForSeconds(this.myanimation[this.fireAnim.name].length);
			this.myanimation.Play(this.ejectshellAnim.name);
			this.myAudioSource.clip = this.readySound;
			this.myAudioSource.Play();
			base.StartCoroutine(this.ejectshell(this.shellejectdelay * 0.5f));
			yield return new WaitForSeconds(this.myanimation[this.ejectshellAnim.name].length * 0.5f);
		}
		else if (this.currentammo <= 1)
		{
			...
			base.StartCoroutine(this.flashthemuzzle());
			this.weaponfirer.fire();
			float randomZ2 = UnityEngine.Random.Range(-0.05f, -0.01f);
			base.transform.localPosition = new Vector3(base.transform.localPosition.x, base.transform.localPosition.y, base.transform.localPosition.z + randomZ2);
			this.fireAudioSource.clip = this.fireSound;
			this.fireAudioSource.pitch = 0.9f + 0.1f * UnityEngine.Random.value;
			this.fireAudioSource.Play();
			this.myanimation.Play(this.fireAnim.name);
	...
	private IEnumerator setreload()
	{
		...
		this.canaim = true;
		UnityEngine.Debug.Log("aaho coming");
		this.myanimation[this.reloadAnim.name].speed = 1.5f;
		yield return new WaitForSeconds(this.myanimation[this.reloadAnim.name].length * 0.2f);
		...
		yield return new WaitForSeconds(this.myanimation[this.reloadAnim.name].length * 0.3f);
		this.myAudioSource.clip = this.readySound;
		this.myAudioSource.loop = false;
		this.myAudioSource.volume = 1f;
		this.myAudioSource.Play();
		this.myanimation[this.ejectshellAnim.name].speed = 2f;
		this.myanimation.Play(this.ejectshellAnim.name);
		yield return new WaitForSeconds(this.myanimation[this.ejectshellAnim.name].length * 0.25f);
