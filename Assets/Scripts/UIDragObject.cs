// dnSpy decompiler from Assembly-CSharp.dll class: UIDragObject
using System;
using UnityEngine;
		...
		if (base.enabled && NGUITools.GetActive(base.gameObject) && this.target != null)
		{
			if (pressed)
			{
				if (!this.mPressed)
				{
					...
					this.mPlane = new Plane(((!(this.mPanel != null)) ? transform.rotation : this.mPanel.cachedTransform.rotation) * Vector3.back, UICamera.lastWorldPosition);
				}
			}
