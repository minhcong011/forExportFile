// dnSpy decompiler from Assembly-CSharp.dll class: UIInput
using System;
...
public class UIInput : MonoBehaviour
{
	...
	public string Validate(string val)
	{
		...
		foreach (char c in val)
		{
            ...
            if (this.onValidate != null)
			{
				...
				ch = this.onValidate(stringBuilder.ToString(), stringBuilder.Length, c);
			}
			else if (this.validation != UIInput.Validation.None)
			{
				ch = this.Validate(stringBuilder.ToString(), stringBuilder.Length, c);
			}
			if (ch != '\0')
			{
	...
	private void Update()
	{
		if (this.isSelected)
		{
			...
			{
				if (this.selectOnTab != null && UnityEngine.Input.GetKeyDown(KeyCode.Tab))
				{
