// dnSpy decompiler from Assembly-CSharp.dll class: MouseLookingMachine
using System;
using UnityEngine;

public class MouseLookingMachine : MonoBehaviour
{
	private void Awake()
	{
		this.bounds = new Rect(0f, 0f, (float)(Screen.width / 6), (float)Screen.height);
	}
	...
	private void Start()
	{
		...
		this.cId = Singleton<GameController>.Instance.refAllController.getCurrentControllerId();
	...
	private void Update()
	{
		if ((this.cId != 1 && this.cId != 2 && this.cId != 3 && this.cId != 4) || this.refGameSceneController.isGameOver || this.refGameSceneController.isGameCompleted || !this.refGameSceneController.isGameStarted || this.cId != this.machineId)
		{
			return;
		}
		...
		for (int i = 0; i < UnityEngine.Input.touchCount; i++)
		{
			...
			if (touch.position.x < (float)(Screen.width / 2) - (float)Screen.width / 2.1f)
			{
		...
		this.rotationX = MouseLookingMachine.ClampAngle(this.rotationX, this.minimumX, this.maximumX);
		this.rotationY = MouseLookingMachine.ClampAngle(this.rotationY, this.minimumY, this.maximumY);
		...
		if (this.bothAxis)
		{
			base.gameObject.transform.localRotation = this.originalRotation * rhs * rhs2;
			base.transform.localEulerAngles = new Vector3(base.transform.localEulerAngles.x, base.transform.localEulerAngles.y, 0f);
		}
		else if (this.forRoot != null)
		{
			base.transform.localRotation = this.originalRotation * rhs2;
			this.forRoot.localRotation = this.originalRotation * rhs;
			this.forRoot.localEulerAngles = new Vector3(this.forRoot.localEulerAngles.x, 0f, 0f);
	...
	public bool leftTouch = true;

	public bool bothAxis = true;
